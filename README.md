...
Coze (Default)	Coze AI Platform	coze.cn / coze.com
Volcengine	火山引擎/豆包/Seedance	console.volcengine.com
OpenAI	DALL-E	platform.openai.com
...
export IMAGE\_API\_TYPE=coze
export COZE\_API\_KEY="your-coze-api-key"
```
Option B: Use Volcengine (火山引擎/豆包/Seedance)
```bash
# Set environment variables
export IMAGE\_API\_TYPE=volcengine
export VOLCENGINE\_API\_KEY="your-volcengine-api-key"
```
Option C: Use OpenAI
```bash
# Set environment variables
export IMAGE\_API\_TYPE=openai
export OPENAI\_API\_KEY="your-openai-api-key"
```
Option D: Use Stability AI
```bash
# Set environment variables
export IMAGE\_API\_TYPE=stability
export STABILITY\_API\_KEY="your-stability-api-key"
```
...
# Edit .env file
IMAGE\_API\_TYPE=coze
COZE\_API\_KEY=your-api-key
```
Get API Keys
Platform	URL	Notes
...
# 2. Configure API key
export COZE\_API\_KEY="your-api-key"

# 3. Run
python weather\_character.py
...
`mood`	开心	`DEFAULT\_MOOD`
`location`	常州	`DEFAULT\_LOCATION`
`timeout`	300s	`TIMEOUT`
`schedule\_time`	07:30	`scheduler.py`
Available Moods
...
├── weather\_character.py   # Core skill code
├── image\_api.py           # Multi-API wrapper
├── morning\_dialog.py      # Interactive dialog
...
export COZE\_API\_KEY="your-api-key"
# OR for Volcengine
export VOLCENGINE\_API\_KEY="your-api-key"
```
...
```bash
export IMAGE\_API\_TYPE=volcengine  # or openai, stability
export VOLCENGINE\_API\_KEY="your-key"
```
---
<a name="中文"></a>
中文
...
```bash
export IMAGE\_API\_TYPE=coze
export COZE\_API\_KEY="你的Coze密钥"
```
方式二：使用火山引擎（豆包/Seedance）
```bash
export IMAGE\_API\_TYPE=volcengine
export VOLCENGINE\_API\_KEY="你的火山引擎密钥"
```
方式三：使用 OpenAI
```bash
export IMAGE\_API\_TYPE=openai
export OPENAI\_API\_KEY="你的OpenAI密钥"
```
方式四：使用 .env 文件
```bash
# 复制模板
...
设置你的 API Key：
```bash
export COZE\_API\_KEY="你的密钥"
# 或使用火山引擎
export VOLCENGINE\_API\_KEY="你的密钥"
```
"图片生成失败"
检查 API Key 是否有效
检查 API 配额
...
如何切换 API？
```bash
export IMAGE\_API\_TYPE=volcengine  # 或 openai, stability
export VOLCENGINE\_API\_KEY="你的密钥"
```
---
许可证
...
---
联系方式 / Contact
如有任何需求或建议，欢迎联系微信：waffle105
For any questions or suggestions, feel free to contact via WeChat: waffle105
<img src="assets/image.png" alt="WeChat QR Code" width="200">
